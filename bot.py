...
async def steal(interaction: discord.Interaction, user: discord.User):
    ...
    if random.random() < probability:
        ...
        embed = discord.Embed(title="Vol échoué", description=f"<@{robber_id}> s'est fait chopper entrain de voler <@{victim_id}> et a perdu {loss} <:AploucheCoin:1286080674046152724> !", color=color_red)
        await interaction.response.send_message(embed=embed)
...
async def withdraw(interaction: discord.Interaction, amount: int):
    ...
    if bank >= amount:
        ...
        embed = discord.Embed(title="Erreur", description="Vous n'avez pas assez d'argent en banque.", color=color_red)
        await interaction.response.send_message(embed=embed)
...
async def leaderboard(interaction: discord.Interaction):
    try:
        ...
        embed = discord.Embed(title="Erreur", description=f"Erreur : {str(e)}", color=color_red)
        await interaction.response.send_message(embed=embed)
...
async def transaction_history(interaction: discord.Interaction, user: discord.User = None):
    try:
        ...
        if data:
            embed = discord.Embed(title="Historique des transactions", description=f"Historique des transactions de {user.mention} :", color=color_green)
...
async def delete_account(interaction: discord.Interaction, user: discord.User):
    ...
    message = await interaction.response.send_message(embed=embed, view=DeleteAccView())
